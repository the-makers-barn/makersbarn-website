...
const NICHE = AgendaNiche.COACHING
const ROUTE = Route.COACHING_RETREAT_AGENDA_BUILDER
...
export default async function CoachingRetreatAgendaBuilderRoute({ params }: PageProps) {
  const { locale } = await params
  const validLocale = getValidLocale(locale)
  const t = await getServerTranslations(validLocale)
  const nicheLabel = t.tools.agenda.nicheLabels[NICHE]
